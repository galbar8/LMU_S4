...
Trains a Mamba model on the PPG-DaLiA dataset for heart rate estimation.

BASIC USAGE:
    python train_ppg_mamba.py --device cuda --amp --evaluate

PROGRAMMATIC USAGE:
    from src.notebooks.ppg.training.train_ppg_mamba import main_fraction, main_multi_fraction
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Train Mamba model on PPG dataset')
    ...
    # PPG-specific data parameters
    parser.add_argument('--win_sec', type=int, default=8,
                        help='Window size in seconds (default: 8)')
    parser.add_argument('--stride_sec', type=int, default=2,
                        help='Stride in seconds (default: 2)')
    parser.add_argument('--fs', type=float, default=100.0,
                        help='Target sampling frequency (default: 100.0)')
    parser.add_argument('--fs_in', type=float, default=64.0,
                        help='Input sampling frequency (default: 64.0)')
    ...
    parser.add_argument('--min_delta', type=float, default=0.01,
                        help='Minimum change to qualify as improvement (default: 0.01)')
    ...
    parser.add_argument('--droppath_final', type=float, default=0.1,
                        help='Final droppath rate (default: 0.1)')
    parser.add_argument('--layerscale_init', type=float, default=1e-2,
                        help='Layer scale initialization (default: 1e-2)')
    ...
    parser.add_argument('--save_dir', type=str, default='./runs/ppg_mamba_task',
                        help='Directory to save checkpoints (default: ./runs/ppg_mamba_task)')
...
def build_config(args_parsed):
    ...
    args: Dict[str, Any] = {
        ...
        "data_loader_kwargs": {
            ...
            "fs_in": args_parsed.fs_in,
            "fs": args_parsed.fs,
            "win_sec": args_parsed.win_sec,
            "stride_sec": args_parsed.stride_sec,
            "do_bandpass": True,
            "low_hz": 0.5,
            "high_hz": 8.0,
        },
        ...
        "droppath_final": args_parsed.droppath_final,
        "layerscale_init": args_parsed.layerscale_init,
        "residual_gain": 1.0,
        "pool": "mean",
...
def train(args: Dict[str, Any]):
    ...
    print(f"Best MAE: {best_metric:.4f}")
...
def main_fraction(fraction: float = 0.1, epochs: int = 100, device: str = "auto",
    ...
    args: Dict[str, Any] = {
        ...
        "data_loader_kwargs": {
            ...
            "subject_val": VAL_SUBJECT,
            "subject_test": TEST_SUBJECT,
            "fs_in": kwargs.get("fs_in", 64.0),
            "fs": kwargs.get("fs", 100.0),
            "win_sec": kwargs.get("win_sec", 8),
            "stride_sec": kwargs.get("stride_sec", 2),
            "do_bandpass": True,
            "low_hz": 0.5,
            "high_hz": 8.0,
        },
        ...
        "save_dir": kwargs.get("save_dir", f"./runs/ppg_mamba_task_frac_{int(fraction*100)}"),
        ...
        "droppath_final": kwargs.get("droppath_final", 0.1),
        "layerscale_init": kwargs.get("layerscale_init", 1e-2),
        "residual_gain": 1.0,
        "pool": "mean",
...
def main_multi_fraction(fractions: list[float] = None, epochs: int = 100, device: str = "auto",
    ...
    Args:
        ...
        epochs: Number of epochs for each fraction (default: 100)
        device: Device to use (default: "auto")
        evaluate_model: Whether to evaluate after training (default: True)
    ...
    for frac in fractions:
        ...
        args: Dict[str, Any] = {
            ...
            "wd": kwargs.get("wd", 1e-4),
            "amp": kwargs.get("amp", amp_enabled),
            "save_dir": f"./runs/ppg_mamba_task_frac_{int(frac*100)}",
            "warmup_epochs": kwargs.get("warmup_epochs", 5),
            "patience": kwargs.get("patience", 10),
            "min_delta": kwargs.get("min_delta", 0.01),
            "d_model": kwargs.get("d_model", 128),
